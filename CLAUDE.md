...
- **Phase 4 — API & ecosystem:**
  ...
  - ~~**Drop-in driver**~~ — **done.** Two public surfaces, both GPU-verified:
    ...
    - **`fsr3(scene, camera, options)`** (`src/FSR3Node.ts`) — the declarative TSL node ("the future" surface per Dennis): `post.outputNode = fsr3(scene, camera)`. A `TempNode` that lazily builds an `FSR3Pass` in `setup(builder.renderer)`, drives it in `updateBefore`, and exposes the output via `passTexture`. No new deps (native TSL, not pmndrs). Owns the scene render itself like `TAAUNode` — for a low-res *composited* input, drive `FSR3Upscaler` directly. Color path: the boot renderer's `NoToneMapping` + `LinearSRGBColorSpace` makes the PostProcessing output transform identity, so the node passes its already-sRGB texture straight through. Examples `07-tsl-node` + `08-tsl-compose` (`fsr3(...).mul(vignette)`). **Note:** three renamed `PostProcessing` → `RenderPipeline` (deprecation warning only).
