...
- **Phase 4 — API & ecosystem:**
  ...
  - ~~**Drop-in driver**~~ — **done.** Two public surfaces, both GPU-verified:
    ...
    - **TSL nodes** (`src/FSR3Node.ts`, native TSL, no pmndrs dep) — "the future" surface. **One node, one code path** — modelled on three's own `FSR1Node` / `TAAUNode`:
      ...
      - **Smart jitter default** (`FSRConfig.jitter`, `FSR3NodeOptions.jitter`): jitter buys *reconstruction* (detail beyond render res) but only if the input is re-rendered under the jittered projection each frame. `fsr3()` consumes whatever texture you give it and can't guarantee that, so it defaults jitter **off** (still reprojects/accumulates/denoises — just no sub-pixel offset, no smear risk). `fsrScene()`/`FSR3Pass` own the render, so they default jitter **on** for free reconstruction. When jitter is off, `beginFrame` no-ops the `setViewOffset` and the jitter constants stay zero; the node skips the jitter hook + velocity compensation and treats depth/velocity as optional. `09-kitchen-sink` has a toggle to A/B it on the same in-graph pipeline.
