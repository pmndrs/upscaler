...
- **Phase 4 — API & ecosystem:**
  ...
  - ~~**Drop-in driver**~~ — **done.** Two public surfaces, both GPU-verified:
    ...
    - **TSL nodes** (`src/FSR3Node.ts`, native TSL, no pmndrs dep) — "the future" surface. Two forms, split on a hard architectural line — **owning the render vs consuming inputs**:
      - **`fsrScene(scene, camera, options)`** (`FSR3SceneNode`) — GPU-verified. Owns the render via `FSR3Pass`, so `beginFrame`/`endFrame` jitter is correct trivially. The clean drop-in: `post.outputNode = fsrScene(scene, camera)`. Examples `07-tsl-node`, `08-tsl-compose` (`.mul(vignette)`).
      - **`fsr3(color, depth, velocity, camera, options)`** (`FSR3Node`) — composable, consumes reduced-res texture nodes. **Two gotchas** (learned the hard way — first attempt rendered black): (1) it does NOT render its inputs (they're read imperatively in `updateBefore`, not shader-graph deps), so *something* must render them each frame; (2) jitter is applied via the `renderPipeline.context.onBeforeRenderPipeline` hook, so it only lands if the inputs render **inside** the post render, not earlier in the user's loop. For an imperative SSGI-style pipeline (composite into a low-res RT yourself), the raw `FSR3Upscaler` is still the right tool and the proven path — `examples/06-screenspace-gi` stays on it. `FSRConfig` gained `renderWidth`/`renderHeight` so the composable node matches an externally-sized input exactly.
      ...
      - **Open design question for Dennis:** whether to make the composable node force its inputs to render in-graph (a dependency hack) so SSGI-in-a-graph jitters correctly, or leave imperative pipelines on the raw upscaler.
